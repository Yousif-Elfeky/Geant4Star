...
protected:
  ...
  std::map<std::string, int> mHitCounts;// added to show sub-detector hits
  ...
  int AddHits( std::string name, std::vector<std::string> volumes, std::string gname, F sd2table ) {
    ...
    int nhits = 0;
    std::map<std::string, int> nHitscurrent;// added to show sub-detector hits
    ...
    for ( auto v : volumes ) {
      ...
      sds.push_back(sd);
      int volHits = sd->numberOfHits(); 
      
      //(CALH:CSDA)
      std::string key = name + ":" + v; 
      
      // Fill the map if there are hits
      mHitCounts[key] += volHits; 
      mHitCounts["ALL"] += volHits;
      nHitscurrent[key] += volHits;
    }
    ...
    for (const auto& kv : nHitscurrent)
      {
        const std::string& volName = kv.first;
        int volHits = kv.second;
        LOG_INFO << "(" << volName << ") adding number of hits = " << volHits << endm;
      }
    if ( nhits > 0 ) {
