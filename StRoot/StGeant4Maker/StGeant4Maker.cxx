...
struct SD2Table_EMC {
  void operator()( StSensitiveDetector* sd, St_g2t_emc_hit* table, St_g2t_track* track ) {
	...
	trk->n_hca_hit++;
      else if ( sdname == "CSDA" ) // why not CSHI???
  trk->n_smd_hit++;  // Barrel SMD
      else if ( sdname == "EXSE" ) // || sdname == "EHMS"?? 
  trk->n_esm_hit++; // Endcap SMD
    }
  } 
} sd2table_emc; 
...
int StGeant4Maker::Finish() {
  ...
  LOG_INFO << "Total hits per sensitive volume" << endm;
  for ( auto kv : mHitCounts ) {

    std::string name = kv.first;
    double      nhits = kv.second;

    LOG_INFO << name.c_str() << " nhits = " << nhits << endm;
