...
// 	  cout << "p[2] :" << rchHitTable[0].p[2] << endl;
    ...
    long nVerticesBound = (long)mCurrentMcEvent->vertices().size();

    for (long itrk=0; itrk<NTracks; itrk++) {
        
        // 1. Calculate Stop Vertex Index (0-based)
        iStopVtxId = (trackTable[itrk].stop_vertex_p) - 1;
                    
        // Bounds Check BEFORE accessing vtemp for primaryFlag
        if (iStopVtxId >= 0) {
            if (iStopVtxId < nVerticesBound) {
                if (vtemp[iStopVtxId].primaryFlag == 1) {
                    nThrownTracks++;
                    continue; // Skip thrown tracks
                }
            } else {
                // If it's out of bounds, we can't check the flag. 
                // We assume it's NOT a primary flag skip
                if (Debug()) { 
                    LOG_WARN << "StMcEventMaker: Track " << itrk << " StopVtx " << iStopVtxId 
                             << " OOB at check 1 (N=" << nVerticesBound << ")" << endm;
                }
            }
        }    
        ...
        // Fix tracks with garbage geant id(VP)
        ...
        // Ensure track ID is within bounds for ttemp array if needed
        if (trackTable[itrk].id - 1 < NTracks) {
             ttemp[ trackTable[itrk].id - 1 ] = t; 
        }

        mCurrentMcEvent->tracks().push_back(t); 

        if (iStopVtxId >= 0) { // Changed > 0 to >= 0 to include index 0
            if (iStopVtxId < nVerticesBound) { // Changed <= to < (0-based indexing is better)
                StMcVertex *v = mCurrentMcEvent->vertices()[iStopVtxId];
                
                if (v) {
                    v->setParent(t);
                    t->setStopVertex(v);
                } else {
                     LOG_WARN << "StMcEventMaker: Track " << itrk << " points to NULL StopVertex " 
                              << iStopVtxId << ". Skipping." << endm;
                }
            } else {
                // Only warn once per corruption
                LOG_WARN << "StMcEventMaker: Track " << itrk << " StopVtx " << iStopVtxId 
                         << " OOB (N=" << nVerticesBound << "). Skipping linkage." << endm;
            }
        }

        iStartVtxId = trackTable[itrk].start_vertex_p - 1;
        
        if (iStartVtxId >= 0 && iStartVtxId < nVerticesBound) {
            v = vtemp[iStartVtxId].vtx; 
            ...
            if (v) {
                ...
                v->addDaughter(t);
                 ...
                 LOG_WARN << "StMcEventMaker: Track " << itrk << " points to NULL StartVertex " 
                          << iStartVtxId << ". Skipping." << endm;
            }
        } else {
             LOG_WARN << "StMcEventMaker: Track " << itrk << " StartVtx " << iStartVtxId 
                      << " OOB (N=" << nVerticesBound << "). Skipping." << endm;
        }

        iItrmdVtxId = (trackTable[itrk].itrmd_vertex_p) - 1;
        
        if (iItrmdVtxId >= 0 && iItrmdVtxId < nVerticesBound) {
            if (vtemp[iItrmdVtxId].vtx) {
                t->intermediateVertices().push_back(vtemp[iItrmdVtxId].vtx);
                vtemp[iItrmdVtxId].vtx->setParent(t);

                // Follow linked list
                int safetyCount = 0;
                while(vertexTable[iItrmdVtxId].next_itrmd_p != 0 && safetyCount < 1000) {
                    safetyCount++;
        ...
        if (iEventGeneratorLabel >= 0 ) {
            if (iEventGeneratorLabel < NGeneratorTracks) {
                ...
                if (trkToErase != mCurrentMcEvent->tracks().end()) {
                    delete *trkToErase; 
            ...
            if ((motherIndex > 0) && (motherIndex <= NTracks)) {
                if (motherIndex > itrk+1) { 
                    if (Debug()) {
                        gMessMgr->Warning() << "Wrong ordering! Track " << itrk+1 << endm;
                    }
                }
                else {
                    t->setParent(ttemp[motherIndex-1]);
                }
            } 
        }
