...
void StMiniMcMaker::buildEmcIndexArray() 
{
  ...
  for (int softIdNum=1; softIdNum<18001; ++softIdNum) mBsmdeIndex[softIdNum]=0;
  for (int softIdNum=1; softIdNum<18001; ++softIdNum) mBsmdpIndex[softIdNum]=0;
  ...
  StEmcDetector* bsmdeDet = mRcEvent->emcCollection()->detector(kBarrelSmdEtaStripId);
  StEmcDetector* bsmdpDet = mRcEvent->emcCollection()->detector(kBarrelSmdPhiStripId);
  if (! bemcDet) return;
  if (Debug()>1) {
  ...
  if (bsmdeDet) {
      ...
      if (bsmdeGeom) {
          for (size_t iMod=1; iMod<=bsmdeDet->numberOfModules(); ++iMod) {
              StSPtrVecEmcRawHit& modHits = bsmdeDet->module(iMod)->hits();
              for (size_t iHit=0; iHit<modHits.size(); ++iHit) {
                  StEmcRawHit* rawHit = modHits[iHit];
                  unsigned int softId = rawHit->softId(3); // 3 is "bsmde"
                  // CORRECTED check to 18001
                  if (softId > 0 && softId < 18001 && !bsmdeGeom->checkId(softId)) { 
                      mBsmdeIndex[softId] = rawHit;
                  }
              }
          }
      }
  ...
  if (bsmdpDet) {
      ...
      if (bsmdpGeom) {
          for (size_t iMod=1; iMod<=bsmdpDet->numberOfModules(); ++iMod) {
              StSPtrVecEmcRawHit& modHits = bsmdpDet->module(iMod)->hits();
              for (size_t iHit=0; iHit<modHits.size(); ++iHit) {
                  StEmcRawHit* rawHit = modHits[iHit];
                  unsigned int softId = rawHit->softId(4); // 4 is "bsmdp"
                  if (softId > 0 && softId < 18001 && !bsmdpGeom->checkId(softId)) { 
                      mBsmdpIndex[softId] = rawHit;
                  }
              }
          }
      }
