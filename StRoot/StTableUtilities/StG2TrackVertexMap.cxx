...
void StG2TrackVertexMap::Reset(St_g2t_track *track, St_g2t_vertex *vertex) {
  ...
  for (Int_t i = 0; i < Nt; i++) {
    ...
    // SAFETY: Track ID bounds
    if (IdT < 0 || IdT >= Nt) continue;

    Int_t IdV = t[i].start_vertex_p - 1;     
    
    // SAFETY: Skip invalid Start Vertex
    if (IdV < 0 || IdV >= NV) {
        // Just map to invalid and continue
        continue;
    }
    ...
    if (IdP >= 0 && IdP < Nt) {
      ...
      // SAFETY: Only check merge if parent vertex is valid
      if (IdVxP >= 0 && IdVxP < NV) {
          TVector3 Vx2(v[IdVxP].ge_x);
          Vx2 -= Vx1;
          
          // SAFETY: Add counter to prevent infinite loop
          int loopCount = 0;
          while ((Vx2.Mag() < 10e-4 && TMath::Abs(tof1-v[IdVxP].ge_tof) < 1e-9)) {
            if (loopCount++ > 50) break; // Break circular loops

            IdV = IdVxP;
            
            IdP = v[IdV].parent_p - 1; 
            if (IdP < 0 || IdP >= Nt) break; // Check bounds
            
            IdVxP = t[IdP].start_vertex_p - 1;  
            if (IdVxP < 0 || IdVxP >= NV) break; // Check bounds
