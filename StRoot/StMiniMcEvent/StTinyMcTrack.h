...
class StTinyMcTrack : public TObject {
 public:
  ...
  // BSMD-Eta
  float bsmdeEnergyMc(size_t index) const {if (index<21) return mBsmdeEnergyMc[index]; else return -999;}
  short bsmdeSoftIdMc(size_t index) const { if (index<21) return mBsmdeSoftIdMc[index]; else return -999;}
  // BSMD-Phi
  float bsmdpEnergyMc(size_t index) const {if (index<21) return mBsmdpEnergyMc[index]; else return -999;}
  short bsmdpSoftIdMc(size_t index) const { if (index<21) return mBsmdpSoftIdMc[index]; else return -999;}
private:
  // mc stuff
